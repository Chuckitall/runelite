	...
	private List<_WidgetItem> getBankItems()
	{
		...
		if (isBankOpen() && bank != null && !bank.isSelfHidden())
		{
			...
			for (int i = 0; i < children.length; i++)
			{
				...
				bankItems.add(_WidgetItem.from(new WidgetItem(child.getItemId(), child.getItemQuantity(), i, bounds, child, null)));
			}
	...
	private List<_WidgetItem> getInvItems()
	{
		...
		if (isBankOpen() && inv != null && !inv.isSelfHidden())
		{
			...
			for (int i = 0; i < children.length; i++)
			{
				...
				Rectangle bounds = child.getBounds();
				bounds.setBounds(bounds.x - 1, bounds.y - 1, 32, 32);
				invItems.add(_WidgetItem.from(new WidgetItem(child.getItemId(), child.getItemQuantity(), i, bounds, child, null)));
			}
