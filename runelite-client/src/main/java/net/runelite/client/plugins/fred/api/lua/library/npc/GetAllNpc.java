...
@Slf4j
public class GetAllNpc extends TwoArgFunction
{
	@Override
	public LuaValue call(LuaValue arg1, LuaValue arg2)
	{
		...
		{
			_NPC[] result = new NPCQuery().idEquals(ids).isWithinDistance(localPlayerLoc.get(), dist).result(client.get()).list.stream().map(_NPC::from).toArray(_NPC[]::new);
