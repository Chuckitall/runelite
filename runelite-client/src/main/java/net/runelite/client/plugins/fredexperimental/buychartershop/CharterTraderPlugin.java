...
@PluginDescriptor(
	name = "Freds Charter trader",
	description = "Does a thing!",
	tags = {"fred", "trade", "striker", "buy"},
	type = PluginType.FRED
)
@Singleton
@Slf4j
public class CharterTraderPlugin extends Plugin
{
	@Inject
	@Getter(AccessLevel.PACKAGE)
	private Client client;
	@Inject
	private CharterTraderConfig config;
	@Inject
	private EventBus eventBus;
	@Inject
	private CharterTraderOverlay sceneOverlay;
	@Inject
	...
	@Provides
	CharterTraderConfig getConfig(ConfigManager configManager)
	{
		return configManager.getConfig(CharterTraderConfig.class);
	}
	...
	{
		if (event.getGroup().equals("chartertrader"))
		{
			updateConfig();
		}
	}
