...
@Slf4j
public class HotKeysScriptEditor extends JFrame
{
	...
	public HotKeysScriptEditor(HotKeysPlugin plugin, HotKeysScript script) throws HeadlessException
	{
		this.plugin = plugin;
		this.script = script;
		...
		DocumentListener documentListener = new DocumentListener() {
			public void changedUpdate(DocumentEvent documentEvent) {
				script.setStatementsFromText(textArea.getText());
			}
		...
		textArea.setText(script.getStatementsAsText());
		...
		addCommandButton.addActionListener(e -> {
			...
			script.setStatementsFromText(text);
			textArea.setText(script.getStatementsAsText());
	...
	{
		plugin.updateConfig();
		setTitle("Script Editor - " + script.getName());
		setAlwaysOnTop(true);
		setVisible(true);
	}
